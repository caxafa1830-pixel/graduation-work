...
テーブル詳細（各カラムの機能を追記）
usersテーブル
name : string — ユーザーの表示名。将来的なランキング・SNS共有機能（9章）で表示名として使う
email : string（ユニーク制約） — ログイン認証に使うメールアドレス。会員登録・ログイン機能で使用し、重複登録を防ぐ
encrypted_password : string — 認証gem（Devise等）が生成する暗号化パスワード。ログイン時の照合に使い、生パスワードは保存しない
money : integer — 所持金。catches（捕獲記録）を売却（換金）すると加算され、luresの購入時にpriceの分だけ減算される。マイページで残高として表示する
luresテーブル（ルアーのマスタデータ）
name : string — ルアー名。ルアーショップ画面・所持ルアー一覧・ルアー選択画面で表示名として使う
action_type : integer（enum） — ルアーの動かし方の分類（ミノー系・バイブレーション系など）。7章のバイト判定ロジック（ルアーの動かし方によってバイト率が変わる仕組み）で、どの誘い方をしやすいルアーかを判定する際に参照する
price : integer — 購入価格。ルアーショップでの購入時に、この金額分だけusers.moneyから差し引く
user_luresテーブル（ユーザーとルアーの中間テーブル）
user_id : bigint（FK） — どのユーザーが所持しているルアーかを表す。マイページの所持ルアー一覧をこのカラムで絞り込む
lure_id : bigint（FK） — 所持しているルアーの種類を表す。出撃前のルアー選択画面で、このテーブルに存在するルアーのみ選択可能にする（未購入のルアーは選べないようにする）
fieldsテーブル（釣り場のマスタデータ）
name : string — 釣り場の名称（例：河口、堤防）。フィールド選択画面での表示名
description : text — 釣り場の特徴の説明文。フィールド選択画面で、その場所の傾向（例：潮の影響を受けやすい等）をユーザーに伝えるために表示する
catchesテーブル（捕獲記録：図鑑・自己ベストの元データ）
user_id : bigint（FK） — 誰が釣った記録かを表す。自己ベストやランキングは、このカラムでユーザーごとに絞り込んでcatchesを集計（MAX/SUM）して算出する
field_id : bigint（FK） — どこで釣ったかを表す。図鑑画面でフィールドごとの釣果を振り返る際の絞り込みに使う
lure_id : bigint（FK） — どのルアーで釣ったかを表す。どのルアーがどんな釣果につながったかを図鑑で振り返れるようにする
size_cm : decimal — 釣ったシーバスのサイズ。リザルト画面の表示、スコア計算、自己ベスト（MAX）の算出に使う
weight_g : decimal — 釣ったシーバスの重さ。リザルト画面の表示に使う（将来的な重量ランキング拡張にも流用できる）
tide_type : integer（enum） — 釣った時点の潮の種類（大潮・中潮・小潮）。本リリースで導入する「潮によって釣れやすさが変わる」仕様の実績データとして記録し、図鑑で振り返れるようにする
caught_at : datetime — 釣った日時。捕獲記録の時系列表示や、期間別のランキング集計に使う
